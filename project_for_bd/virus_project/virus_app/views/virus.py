    ...
    if request.method == 'POST':
        ...
        for item in items:
            if item[0].startswith(delete_from_fav_str):
                ...
                return redirect('scroll_epidemic', epidemiology_pk=epidemiology_pk)

    viruses = Virus.objects.base_information()
    two_viruses_in_row = create_two_elems_in_row(viruses)
    fav_viruses = []
    if (request.user.is_active):
        fav_viruses = Favourites.objects.get_all_viruses_by_username(request.user)
    return render(request, 'virus_app/virus/scroll_virus.html', {'two_viruses_in_row' : two_viruses_in_row,
...
def edit_virus_info(request, pk):
    ...
    if request.method == 'POST':
        Epidemiology.objects.post_changes_in_epidemiology(virus.epidemiology.pk, request.POST)
        try:
        ...
        #return render(request, 'virus_app/virus/more_virus_info.html', {'virus' : virus})

    epidemiology, other_epidemiology = Epidemiology.objects.get_edit_data(virus.epidemiology.pk)

    return render(request, 'virus_app/virus/edit_virus_info.html', {'virus' : virus,
...
def more_virus_info(request, pk):
    ...
    epidemiology = Epidemiology.objects.get_printable_data(virus.epidemiology.pk)
